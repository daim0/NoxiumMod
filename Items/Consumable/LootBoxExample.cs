...
using NoxiumMod.Items.Accesories;
using NoxiumMod.Items.Armor.HorizonStrider;
using NoxiumMod.Items.Weapons.Moonless;
using Terraria.ModLoader;
...
namespace NoxiumMod.Items.Consumable
{
	public class LootBoxExample : LootBox
	{
		...
		{
			DisplayName.SetDefault("Moonless Crate");
			Tooltip.SetDefault("'Totally banned in Norway'");
		}

		public override string Texture
		{
			get { return ("NoxiumMod/Items/Consumable/MoonlessCrate"); }
		}
	...
	public class LootBoxOpenExample : LootBoxOpen
	{
		...
		protected override float speed => 0.22f; //How fast the items scroll be, be mindful of this value to not go over the max items!
		...
		protected override int itemsVisible => 6; //How many items would be visble left as well as right, so really it's twice this value
		...
		public override void SetDefaults()
		{
			...
			projectile.timeLeft = 400 + Main.rand.Next(-100, 300);//Adjust how long the ticker goes, again be mindful of the max item count
        ...
        {
            Texture2D ticker = mod.GetTexture("Items/Consumable/LineSelect");
